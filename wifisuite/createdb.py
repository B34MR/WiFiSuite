...
def createdatabase(db_path):
		try:
			...
			cur.execute('''CREATE TABLE eapcreds(
				ID INTEGER PRIMARY KEY, 
				identity text,
				password text,
				essid text,
			...
			cur.execute('''CREATE TABLE wpakeys(
				ID INTEGER PRIMARY KEY, 
				password text,
				essid text,
				Timestamp DATETIME DEFAULT CURRENT_TIMESTAMP
			...
			conn.commit()
			conn.close()
			print(' ['+colors.blue +'i'+colors.normal+']'+' Database instantiated')
		
		except sqlite3.Error as e:
			print('['+colors.red +'!'+colors.normal+']'+' Cound not create database: %s' % e)
...
def dbcheck():
	...
	# If 'eap.db' is not found in existing directory ~/.eapSuite/' 
	# Create 'eap.db' in directory ~/.eapSuite/' 
